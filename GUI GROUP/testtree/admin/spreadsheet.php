...
if($_POST)
{
	...
	if($_POST['lastPage']){
		...
		//Make sure you can't go back from 1st page
		...
		//Make sure index is correct for first and back 
			...
			$globalRowIndex-=($rowsPerPage*(($_POST['lastPage']-$currentPage)+1));

		//Make sure index is correct for last and next
		else if($_POST['lastPage']<$currentPage)
			$globalRowIndex+=($rowsPerPage*(($currentPage-$_POST['lastPage'])-1));

		//Make sure you can't go forward from last page
...
print <<<_HTML_
		<FORM method = "post" action = "http://localhost/test3.php" >
